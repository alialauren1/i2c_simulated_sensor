...
void I2C0_IRQHandler(void)
{
  ...
  if (pending & I2C_IF_ADDR) {
    ...
    if (addr_byte & 0x01) {
      ...
      sensor_state = STATE_RECEIVE_CMD;
    }
  ...
  if (pending & I2C_IF_RXDATAV) {
    uint8_t cmd = I2C0->RXDATA;

    if (sensor_state == STATE_RECEIVE_CMD && cmd == 0xAC) {
  ...
  if (pending & I2C_IF_ACK) {
    I2C_IntClear(I2C0, I2C_IFC_ACK);
    if (sensor_state == STATE_SEND_DATA && tx_index < 5) {
      I2C0->TXDATA = tx_byte[tx_index++];
    }
  }
